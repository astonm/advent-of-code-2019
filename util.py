...
def read_file(input, delim="\n", strip=True):
    lines = [l for l in input.read().strip("\n").split(delim) if l.strip()]
    if strip:
...
class Grid:
    ...
    def __hash__(self):
        return hash(tuple(tuple(l) for l in self.lines))
...
class GridN:
    ...
    def print(self, sep="", vsep="\n", axis_order=None):
        ...
        if axis_order is None:
            axis_order = list(range(self.dim))[::-1]
...
class Vector(list):
    ...
    def __str__(self):
        return list.__str__(self).replace("[", "<").replace("]", ">")

    def __repr__(self):
        return list.__repr__(self).replace("[", "<").replace("]", ">")
